...
// see https://github.com/ARMmbed/mbed-os-example-crash-reporting/blob/master/main.cpp
#include "mbed.h"
        ...
        if (err_status < 0) {
            ...
            if (err_status == MBED_ERROR_HARDFAULT_EXCEPTION) {
                ...
                printf("    SP   : 0x%X\n"
                       ...
                       "    PSP  : 0x%X\n"
                       "    MSP  : 0x%X\n",
                       (unsigned int)fault_ctx.SP_reg,
                       (unsigned int)fault_ctx.LR_reg,
                       (unsigned int)fault_ctx.PC_reg,
                       (unsigned int)fault_ctx.xPSR,
                       (unsigned int)fault_ctx.PSP,
                       (unsigned int)fault_ctx.MSP);
            }
            else {
